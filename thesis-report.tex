...
\item \textbf{Continuum toggle:} This toggle allows users to view spectra and templates with or without continuum. Continuum subtraction algorithms are detailed in section \textcolor{red}{PUT THE SECTION IN HERE}.
...
The processing algorithm is responsible for presenting the end user with a spectra to manually redshift as quickly as possible, with defects such as blanks and cosmic rays automatically removed, and to have the spectra available with and without continuum. 
...
This process can be described mathematically via the following formula, where $s_i$ represents the $i$th element of the spectra, $v_i$ represents the $i$th element of the spectra variance, and $n$ is the floored half window width, such that a window of seven pixels would give $n=3$.
\begin{align}
s_i = 
\begin{cases}
	\brac{\sum\limits_{j=i-n}^{i+n} \begin{cases}1 & \text{if } f(j) \\ 0 & \text{otherwise} \end{cases}}^{-1}\brac{\sum\limits_{j=i-n}^{i+n} \begin{cases}s_j & \text{if } f(j) \\ 0 & \text{otherwise} \end{cases}} & \text{if } f(i) \\
	s_i & \text{otherwise}
\end{cases}\\
